...
const serviceCallService = {
  createServiceCall: async (serviceData) => {
    ...
    const result = await sequelize.query(
      "CALL CreateServiceCall(:Service_Call_Description, :Address, :Post_Code, :Contact_Phone, :Contact_Email, :Site_Status, :Created_By, :Status,:Contact_First_Name,:Contact_Last_Name)",
      {
        replacements: {
          ...
          Contact_First_Name: serviceData.Contact_First_Name,
          Contact_Last_Name: serviceData.Contact_Last_Name,
        },
  ...
  updateServiceCall: async (servicecallId, serviceCallData) => {
    return sequelize.query(
      "CALL UpdateServiceCall(:id, :description, :address, :postCode, :contactPhone, :contactEmail, :sitestatus, :status,:Contact_First_Name,:Contact_Last_Name)",
      {
        replacements: {
          id: servicecallId,
          ...
          status: serviceCallData.Status || "Not Started",
          Contact_First_Name: serviceCallData.Contact_First_Name,
          Contact_Last_Name: serviceCallData.Contact_Last_Name,
        },
        type: sequelize.QueryTypes.RAW,
      }
    );
  ...
  createServiceSubtask: async (serviceId, subtaskId) => {
    return sequelize.query(
      "CALL CreateServiceCallSubtask(:Service_Call_ID, :Subtask_ID)",
      {
        replacements: {
  ...
  deleteServiceTask: async (Task_Id) => {
    return sequelize.query("CALL DeleteServiceCallTask(:taskId)", {
      replacements: { taskId: Task_Id },
      type: sequelize.QueryTypes.RAW,
    });
  },
